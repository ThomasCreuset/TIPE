...
let affiche_originale = ref true;; (*Pour afficher la structure d'origine*)
let affiche_modifiee = ref true;; (*Pour afficher la structure modifiee*)
...
let couleurs_noeuds noeuds forces =
	...
	let tab_couleurs = Array.map (fun norme_force -> 
					...
					rgb teinte 0 (255 - teinte) ) normes_forces 
	in
	tab_couleurs;; (*Censé faire un dégradé du bleu au rouge*)

let couleurs_aretes aretes forces_axiales =

  let max_force  = Array.fold_left max forces_axiales.(0) forces_axiales
  and min_force = Array.fold_left min forces_axiales.(0) forces_axiales in
  let max_val_abs = max (abs_float max_force) (abs_float min_force) in
	let tab_couleurs = Array.map (fun norme_force -> 
		if norme_force = 0. then rgb 0 0 0
		else
		begin
		if norme_force < 0. then
			begin
			let teinte = int_of_float (map (-.max_val_abs) 0. 0. 255. norme_force) in print_int teinte; print_newline(); rgb 0 0 teinte
			end
		else
			begin
			let teinte = int_of_float (map 0. max_val_abs 0. 255. norme_force) in print_int teinte; print_newline();rgb teinte 0 0
			end
		end
		) forces_axiales
	in
...
let epaisseurs_elements elements = 
	...
	and min_epaisseur = 2. in(*Constantes d'épaisseurs des traits*)
	let max_epaisseur = (max_section/. min_section) *. min_epaisseur in
...
let make_items_affichables elements noeuds forces deplacements forces_axiales = 
	...
	and tab_aretes_deplacees = Array.map2 (fun (i1,i2,young,section) (epaisseur,couleur) -> Arete(noeuds_depl.(i1),noeuds_depl.(i2),epaisseur,couleur) ) elements epaisseurs_couleurs
	and tab_noeuds_deplaces = Array.map2 (fun point couleur -> Noeud(point,7,couleur)) noeuds_depl couleurs_n in
	(* let items_originels = Array.append tab_aretes_originelles tab_noeuds_originels
	and items_deplaces = Array.append tab_aretes_deplacees tab_noeuds_deplaces *)
	let items_originels =tab_aretes_originelles
	and items_deplaces = tab_aretes_deplacees in
	items_originels,items_deplaces;;
...
let lecture_fichier nomFichier =
	...
	for i = 0 to nb_elements-1 do
		ligne:=input_line fichier;
		let element,force_axiale = Scanf.sscanf !ligne "%d;%d;%f;%f;%f" (fun i1 i2 module_young section force_axiale -> (i1,i2,module_young,section),force_axiale) in
			ajoute elements element;
			ajoute forces_axiales force_axiale;
	done;
	close_in fichier;
...
let en_sync_items items_originels items_deplaces =
	auto_synchronize false;
	display_mode false;
	while true do
		clear_graph ();
		let liste_items = ref [] in
		if !affiche_originale then liste_items := items_originels::(!liste_items);
		if !affiche_modifiee  then liste_items := items_deplaces::(!liste_items);
		...
		if key = 'r' then affiche_originale := (not !affiche_originale);
		if key = 'f' then affiche_modifiee  := (not !affiche_modifiee);
	done;;

(*Fonction main : récupere les tableaux et lance la fonction en_sync_items.*)
let main () =
	let noeuds,deplacements,forces,elements,forces_axiales = lecture_fichier "resultat.txt" in
	print_string "Nombre d'éléments : ";
	print_int (Array.length elements); print_newline();
	let items_originels,items_deplaces = make_items_affichables elements noeuds forces deplacements forces_axiales in
	en_sync_items items_originels items_deplaces;;
